...
public class AddEmpController extends AdminDeptEmpInfoController {
    ...
    void cancel(MouseEvent event) {
        try {
            ...
            Logger.getLogger(AddEmpController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    void submit(MouseEvent event) {
        ...
        try {
            this.openConnection();
            preSql = this.conn.prepareStatement("SELECT * FROM employee WHERE EMP_NUM = ? OR EMP_EMAIL = ?");
            preSql.setInt(1, Integer.parseInt(in_code.getText()));
            preSql.setString(2, in_email.getText());
            
            ResultSet rs = this.getData(preSql);
            if (rs.next()) {
                lb_err.setText("Mã nhân viên hoặc Email đã tồn tại !");
                lb_err.setVisible(true);
                return;
            } else {
                lb_err.setVisible(false);
            ...
            preSql = this.conn.prepareStatement("INSERT INTO employee (EMP_NUM, EMP_LNAME, EMP_FNAME, EMP_MNAME, EMP_EMAIL, EMP_JOBCODE, EMP_HIREDATE, EMP_DOB) VALUES "
                    + "(?, ?, ?, ?, ?, ?, ?, ?)");
            preSql.setInt(1, emp_new.getEMP_NUM());
            preSql.setString(2, emp_new.getEMP_LNAME());
            preSql.setString(3, emp_new.getEMP_FNAME());
            preSql.setString(4, emp_new.getEMP_MNAME());
            preSql.setString(5, emp_new.getEMP_EMAIL());
            preSql.setString(6, emp_new.getEMP_JOBCODE());
            preSql.setDate(7, emp_new.getEMP_HIREDATE());
            preSql.setDate(8, emp_new.getEMP_DOB());

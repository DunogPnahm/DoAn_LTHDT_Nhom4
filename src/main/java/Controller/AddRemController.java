...
public class AddRemController extends AdminRemController {
    ...
    void cancel(MouseEvent event) {
        try {
            ...
            Logger.getLogger(AddRemController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    void submit(MouseEvent event) {
        ...
        try {
            ...
            preSql = this.conn.prepareStatement("SELECT * FROM reminder WHERE REM_NUM = ?;");
            preSql.setInt(1, Integer.parseInt(in_code.getText()));
            
            ResultSet rs = this.getData(preSql);
            if (rs.next()) {
                lb_err.setText("Mã thông báo đã tồn tại!");
                lb_err.setVisible(true);
                return;
            } else {
                lb_err.setVisible(false);
            ...
            this.openConnection();
            preSql = this.conn.prepareStatement("INSERT INTO reminder(REM_NUM, REM_DESC, EMP_NUM) VALUES "
                    + "(?, ?, ?)");
            preSql.setInt(1, rem_new.getREM_NUM());
            preSql.setString(2, rem_new.getREM_DESC());
            preSql.setInt(3, rem_new.getEMP_NUM());

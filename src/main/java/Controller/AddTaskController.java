...
public class AddTaskController extends ProfTaskController {
    ...
    void submit(MouseEvent event) {
        ...
        try {
            ...
            preSql = this.conn.prepareStatement("SELECT * FROM task WHERE TASK_CODE = ?;");
            preSql.setString(1, in_code.getText());
            
            ResultSet rs = this.getData(preSql);
            if (rs.next()) {
                lb_err.setText("Mã nhiệm vụ đã tồn tại!");
                lb_err.setVisible(true);
                return;
            } else {
                lb_err.setVisible(false);
            ...
            this.openConnection();
            preSql = this.conn.prepareStatement("INSERT INTO task(TASK_CODE, TASK_DESC, CLS_CODE) VALUES "
                    + "(?, ?, ?)");
            preSql.setString(1, task_new.getTASK_CODE());
            preSql.setString(2, task_new.getTASK_DESC());
            preSql.setString(3, task_new.getCLS_CODE());
